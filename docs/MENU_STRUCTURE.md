...
- 부가세 배지 → 깔끔한 텍스트 (`258e845`)
- 부가세 기본 포함 · 이름 힌트 자동 감지 · 공급 요약 텍스트화 (`cc4ccae`)

**손실추적** (SalesTrendPage LossTrackerTab · 2026-08-06 신규):
- **서브탭 (현황/이력)** · 날짜별 손실 스냅샷·이력·집계 (T-LOSS-HISTORY · `859c37f`)
- 컬럼: 구역·단가·판매가 (`fe08712`, `b18419b`)
- DB: `migrations/loss_tracking_daily.sql` · 서버 라우터: `server/routes/lossTracking.ts`

**카테고리 rename** (전체 프로젝트 · 2026-08-06 · `44459b6`):
- 60일회전 → 60회전 · 90일회전 → 90회전
...
| 5 | **발주요청** | (표시 로직 · orderReqs 있을 때) | 진행중 발주 요청 · **공급사 분류 필터** (2026-08-06 · `25dce84`) |

**진열요청 표 컬럼** (사용자 확정 · 2026-08-05):
- 상품명 · 진열구역 · 담당자 · **창고준비** (버튼) · **진열완료** (버튼) · 날짜
- 창고담당 [준비완료] → 진열담당 [완료] → 관리자 알림

**발주요청 · 실재고 입력** (2026-08-06 갱신):
- 상세 모달 · `InventoryEditModal` 공통 컴포넌트 사용 (`fabd95f`) · 누적 UX · zone별 저장 버튼
- 실재고 컬럼 임시 주석처리 + 손실확정 확인 코멘트 (`639661b`)

**반품필요 · 판매 통합** (2026-08-06 · `058e92d`):
- 이름 정제 표시 + VAT 자동 추론 · 부가세 배지 → 텍스트 (`258e845`)
