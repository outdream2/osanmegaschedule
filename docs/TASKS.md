...
**남은 대기 (2026-08-25 오전 요청)**
1. ✅ **매장구역 선택 Tier 3** · RealMapSelector 모달 통합 (`38b1a8fe`) · 데이터 정합성 100%
2. ✅ **배치구역 불일치** · 매장구역 (StoreZoneMap) 안 새 탭 추가 (`5de30d11`) · TodayStatusPanel 재라우팅
3. ✅ **최근스캔 진열요청 버튼** · 우측 헤더 · [진열요청] · [유통기한임박] 액션 (`f73ba7d6`) · 필터 UI 제거
4. 🟡 **SaveCard 합계** · product_storage.png 톤 적용
5. 🟡 **점심 여전히 표시됨** (버그) · viewport fix 후에도 노출 · 재확인 필요
6. 🟡 **임시저장 복구 프롬프트 작동 안 함** (버그)
7. 🟡 **입고알림 페이지** · v9 목업 개선
8. 🟡 **반품요청서** · 목업 디자인 적용
9. 🟡 **공급사관리** · 반응형 UI 개선
10. 🟡 **반품필요** · 공급사·상품 모두 검색 가능
11. ✅ **직원 등록 · 핸드폰 중복검증** (`111d1aaa`)
12. ✅ **거래처 PATCH · 핸드폰·공급사명 중복검증** (`e6825d62`)
13. 🟡 **반품필요 리스트 · [반품확정] 버튼 추가** · 확정된 것 [반품확정] 페이지에 표시 (신규 기능)
14. 🟡 **결제 메뉴 · 차용입력 페이지 신설** · 공급사·약국 차용기록 (상품·갯수·일자·서명 등 조사 후 페이지 구성)
15. ✅ **메뉴 · 통계 → 판매 · 반품 신규** (`5e5f6e97` · 구조만)
    - 남은 · dpSubTab === "return" → ReturnListPanel 라우팅
    - 남은 · OrderManagePage purchase 서브탭의 반품필요 제거 · 링크 리다이렉트
