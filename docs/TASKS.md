...
### ✅ T-TEST-매입이력-기간필터 · **해결됨 (버그 아님)**
- **원인**: DB에 실제 매입 데이터가 2026-07-20~28 (1주일치)만 있어서 · 6개월 요청해도 반환되는 데이터가 같음
- curl 검증: `from=2026-02-01&supplier=대지인팜` → 170행 · 최대 07-28 · 실제 데이터 범위 확인
- **서버·클라이언트 로직 정상**
- 힌트 UI 추가 예정 (실제 표시 범위 명시)

### T-TEST-매입이력-리스트길이 · 공급사 리스트 10개씩 반응형
- 좌측 공급사 리스트 · 최소 10개 표시되도록 max-h 확대
- 파일: `PurchaseHistoryTab.tsx` (by-vendor leftClassName)

### T-TEST-매입이력-합계접기 · 공급사별 현황 · 합계 부분 접기
- SupplierTab 우측/하단 합계 섹션 · 접기/펼치기 버튼
- 파일: `src/components/StockManagePage/SupplierTab.tsx`
