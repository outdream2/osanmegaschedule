...
## 🔴 근로계약서 대형 재작업 (2026-08-05 · 사용자 여러 지시 통합)

### T-A · 왼쪽 폼 전면 재작성
- 지금 왼쪽 폼 · 다 없애고 새로 구성
- 3단계 · 근로자 정보 → 근무조건 → 희망 세후 수령액
- **희망 세후 입력 시 · 실시간 역산 → 임금구성표 8항목 자동 채움**
- 오른쪽 프리뷰 · 즉시 반영
- 역산 로직: 단일 통상시급 · `세전 = w × (209 + 55.94×1.5 + 22×1.5 + 10) = w × 335.91` · 세후 → 세전 (4대보험 · 소득세 역산) → 시급 유도

### T-B · 계약 기간 layout 정리
- "계약체결일" · "근무시작일" · "종료일 (계약직만)" 줄 맞춤 · 배지 X · 깔끔 텍스트

### T-C · 각 호 내용 · 근로계약서 설정에서 편집 (CMS)
- 임금 단서 5개 · 징계 13개 · 기타 5개 · 개인정보 4분류 · 각 호 내용 편집 가능
- 저장 · settings 테이블 or contract_clauses 신규 테이블

### T-D · 서명 지점 · 이름 자동
- 예: "위의 임금 조항 · 이해하고 동의함 · {employeeName} [서명 pad]" · 형태
- 카테고리별 이해·동의 3개 · 각각 서명 pad + 이름 자동 표시

### T-E · 근무요일 변경 가능 문구
- 계약서에 "근무요일은 갑의 사정에 따라 변경될 수 있으며 을은 정당한 사유 없이 이를 거부할 수 없다" 문구 추가 (또는 유사)
- 평일 기준 (주5일) default

### T-F · 시급 default (완료 · 33fa02a)
- 약사 주중 35,000 · 주말 40,000
- 사원 주중 10,030 · 주말 12,000
- SettingsModal 편집 가능
