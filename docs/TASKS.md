...
- **#24** · ✅ UI 개선 리서치 완료 (`docs/RESEARCH_STAFFMANAGE_UI_2026-08-10.md`)
...
**발주 발송 채널**:
- **#28** · 카카오톡 알림톡 채널 추가 · 발주 모달 (일괄·개별) · SolAPI 클라이언트 스켈레톤 있음 (`448f1ea`) · 사업자 인증·템플릿·env 대기

**신규 (2026-08-10 추가)**:
- **#31** · 발주필요 툴바 · 분류·발주조건 · PC 한줄 · 모바일 2줄
- **#32** · 스캔 실재고입력·상품입고 · SplitPanel 폭 조절 · 공통 CSS ⏸
- **#33** · 스캔 실재고입력 리스트 · 반응형 글씨 확대 · 아이콘 제거 · 매장 spec (ERP 위치) 표시
- **#34** · 스캔 실재고 · 합계 배치 · 위 기존 합계숫자 · 아래 입력창 ⏸
- **#35** · 발주서 모달 · 이전단가 옆 단가 컬럼 제거 · 표 한번에 노출
- **#36** · 발주서 모달 · 소계 앞 "총" 접두 · 글씨 +2 · 리스트 글씨 +2
- **#37** · 발주서 모달 · 현재고·적정재고 컬럼 제거
- **#38** · 매입이력 페이지 상단 툴바 · CARD_BASE 공통 CSS · 다른 페이지와 통일
- **#39** · 발주리스트 · 발주정보 카테고리 그룹 (주문수량·이전사입가·발주금액) + 공급사 셀 제거 + 헤더 VendorCategoryBadge ⏸ (진행 중)
...
- `dd0ef8b` · #21 · vendors 팀장·긴급연락처 마이그레이션
- `b1c86ab` · #19 · 매입이력 상단 기간 chip + 계절 chip 이동 · split 안 제거
- `46c881c` · #21 · vendors 팀장·긴급연락처 UI + 서버 (fallback 지원)
...
- `08bfe8c` · DB dead column 감사 보고 (5건)
- `10a59b5` · DB dead column DROP 마이그레이션
- `377356f` · 공급사 상세 입력창 여백 반 축소
- `e5b5e80` · 사업자번호 중복 검증 (POST·PATCH · 409)
- `8a31360` · 공급요약/결제/기본정보 제목 폰트 +1
- `5a4cba7` · Level 2 탭 (매장 서브탭) 폰트+아이콘 +1
- `f85fd29`·`f2beacf`·`9e3e487` · 발주필요 · 안내문 제거 · 나란히 · 살짝 굵기
- `6f03594`·`c959529`·`68139c5`·`be917d9`·`6834c1b`·`2887249` · 발주필요 컴팩트 정리
- `6e0699a` · 매입이력 embedded · 기간 필터 제거
- `1eb3ea0` · 반품필요 헤더 +3 · 자동조회 안내 제거
- `de68d14` · 스캔 카드 · 그라데이션·장식 원 제거
- `5b43058`·`f560eeb` · 실재고입력 teal / 상품입고 blue
- `fe4b7f5` · ProductSearchInput · 리스트 선택 시 자동 onSelect (확인 버튼 우회)
...
- `432c045` 발주서 · 단가 헤더 2줄 (이전/사입가)
- `68db655` 발주요청 리스트 헤더 폰트 +2
- `3348114` 발주요청 목록 툴바 · 폰트 +2 · 아이콘 확대
- `f98ba72` 발송채널 · 가로 배치 · 카카오톡 기본
- `aed4777` 특이사항·요청메모 · 각 공급사별 · 표 하단 배치
- `bc35d24` 매입이력 · CARD_BASE 상단 통일 · SubTabs 기간필터 제거 · 발주요청 목록 한줄 wrap
- `a78b0cd` 발주요청 툴바 폰트 +1 · 발주필요 공급사·상품명 auto width
