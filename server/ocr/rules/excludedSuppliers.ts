...
//
// ═══════════════════════════════════════════════════════════════════════════
// 프레임워크 재사용 안내 (2026-08-12)
// ═══════════════════════════════════════════════════════════════════════════
// 아래 배열은 **현재 배포 테넌트 (코스트팜/메가타운) 기본값** 입니다.
// 다른 약국에 배포할 때는 · 이 파일을 수정하지 말고 · 환경변수로 override 하세요:
//
//   OCR_RECIPIENT_COMPANY  · 자체 약국 상호 (콤마·파이프 구분)
//   OCR_RECIPIENT_NAMES    · 자체 약국 담당자 이름
//   OCR_RECIPIENT_ADDRESS  · 자체 약국 주소 (일부 조각 · 콤마·파이프 구분)
//   OCR_EXCLUDED_LOGISTICS · 배송사·물류사 (사용 물류사만 남기고 축소)
//   OCR_EXCLUDED_SUPPLIERS · 그 외 추가 제외 (자유)
//   OCR_EXCLUDED_BUSINESS_NUMBERS · 자체 사업자번호 (하이픈 유무 무관)
//
// 이 배열의 값 + env 값이 병합되어 사용됩니다 · env 만으로 충분히 커버됩니다.
// (즉 · 배포 tenant 마다 소스 수정 없이 · .env 만 바꾸면 됨)
// ═══════════════════════════════════════════════════════════════════════════
//
// 관리 방식 (기존):
//   1) 이 파일의 DEFAULT_EXCLUDED_SUPPLIERS 배열을 수정 → 재배포로 즉시 반영
//   2) 환경변수로 추가 확장 가능 (위 안내)
