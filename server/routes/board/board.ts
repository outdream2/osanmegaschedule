...
router.post("/api/board/upload-image", asyncHandler(async (req, res) => {
  ...
  if (!match) throw badRequest("잘못된 data URL");
  const mime = match[1];
  const b64 = match[2];
  const buffer = Buffer.from(b64, "base64");
  // 크기 제한 5MB
  if (buffer.length > 5 * 1024 * 1024) {
    throw new HttpError(413, "이미지 크기 초과 (5MB)");
  }
...
// ── 게시글 목록
router.get("/api/board/posts", asyncHandler(async (req, res) => {
...
// ── 게시글 상세 (댓글 + 이미지 포함)
router.get("/api/board/posts/:id", asyncHandler(async (req, res) => {
  ...
  if (postRes.error) throw new HttpError(500, postRes.error.message);
  if (!postRes.data) throw new HttpError(404, "not found");
...
router.patch("/api/board/posts/:id", asyncHandler(async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) throw badRequest("invalid id");
  const b = req.body ?? {};
...
// ── 게시글 삭제 (작성자·관리자만)
router.delete("/api/board/posts/:id", authorize(9), asyncHandler(async (req, res) => {
  const id = Number(req.params.id);
...
// ── 댓글 수정 (작성자만)
router.patch("/api/board/comments/:id", asyncHandler(async (req, res) => {
  const id = Number(req.params.id);
  const b = req.body ?? {};
  const editorId = Number(b.editor_id ?? 0);
  if (!Number.isFinite(id)) throw badRequest("invalid id");
...
// ── 댓글 삭제
router.delete("/api/board/comments/:id", authorize(2), asyncHandler(async (req, res) => {
  const id = Number(req.params.id);
  const editorId = Number(req.query.editor_id ?? 0);
  const editorLevel = Number(req.query.editor_level ?? 0);
  if (!Number.isFinite(id)) throw badRequest("invalid id");
...
// ── 답변 채택 · 원글 작성자만
router.post("/api/board/comments/:id/accept", asyncHandler(async (req, res) => {
  const id = Number(req.params.id);
  ...
  if (!post || post.author_id !== editorId) throw forbidden("채택 권한 없음");
  // 같은 글 내 기존 채택 해제
...
// ── 반응 (👍 helpful / 👀 seen) 토글
router.post("/api/board/posts/:id/react", asyncHandler(async (req, res) => {
  const postId = Number(req.params.id);
...
router.post("/api/board/cloudinary-signature", asyncHandler(async (_req, res) => {
  const cloudName = process.env.CLOUDINARY_CLOUD_NAME;
  const apiKey = process.env.CLOUDINARY_API_KEY;
  const apiSecret = process.env.CLOUDINARY_API_SECRET;
  if (!cloudName || !apiKey || !apiSecret) {
    throw new HttpError(500, "Cloudinary 미설정");
  }
