...
router.get("/api/leave-requests", async (req, res) => {
  ...
  try {
    let q = supabase.from("leave_requests").select("id, employee_id, employee_name, leave_type, start_date, end_date, reason, status, reviewer_note, reviewed_at, created_at").order("created_at", { ascending: false });
