...
router.get("/api/pharmacist-menu-items", async (req, res) => {
  try {
    ...
    let q = supabase
      ...
      .select("id, tab_key, category_key, title, file_url, file_name, file_size, mime_type, storage_path, storage, sort_order, uploaded_by, uploaded_by_id, updated_at, created_at")
