...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (managerId) {
      ...
      if ((emp?.level ?? 0) < 9) throw forbidden("level 9 이상 관리자만 가능합니다");
    } else {
      throw forbidden("managerId 필요");
    }
    ...
    if (arr.length < 2) throw badRequest("데이터가 부족합니다");
    ...
    if (codeI < 0) {
      throw new HttpError(400, `필수 컬럼(상품코드)을 찾을 수 없습니다. 감지된 헤더: ${headers.join(", ")}`);
    }
    if (dateI < 0 && !inferredTo) {
      throw new HttpError(400, `매입일자 컬럼이 없고 파일명/query 에서도 기간을 파악하지 못했습니다. 파일명(예: xxx_YYYYMMDD_MMDD.xlsx) 형식이거나 ?from=YYYY-MM-DD&to=YYYY-MM-DD 파라미터가 필요합니다.`);
    }
    ...
    let useSimpleInsert = false;
    {
      ...
      if (r.error) {
        if (/relation .* does not exist/i.test(r.error.message)) {
          throw new HttpError(500, "purchase_details 테이블이 없습니다. server/routes/purchase.ts 상단 주석의 SQL 을 Supabase 에서 실행해주세요.");
        }
    ...
    if (inferredFrom && inferredTo) {
      try {
        ...
        if (existingCount > 0 && !forceOverwrite) {
          // 확인 대기 · 클라이언트에게 알림
          throw new HttpError(409, `기간 ${inferredFrom} ~ ${inferredTo} 에 이미 ${existingCount}행 존재. 덮어쓰시겠습니까?`);
        }
        if (existingCount > 0) {
...
router.delete("/api/purchase-details/import-log", authorize(9), asyncHandler(async (_req, res) => {
  await supabase.from("app_settings").upsert(
