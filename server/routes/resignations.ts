...
router.get("/api/resignations", async (req, res) => {
  ...
  try {
    let q = supabase
      ...
      .select("id, employee_id, employee_name, position, hire_date, last_work_date, reason, reason_detail, handover_notes, signature_url, pdf_url, status, reject_reason, approved_by, approved_by_id, approved_at, created_at")
