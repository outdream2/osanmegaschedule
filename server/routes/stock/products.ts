...
// 2026-09-09 · #15 · products.shelf_positions 단일 소스 · optimal_stock 원칙 동일 (사용자 지시)
//   · GET /api/products/shelf-positions-map · { [product_code]: { [location_code]: "332" | null } }
//   · products 마스터에서만 조회 · inventory_checks 스냅샷 폐지
...
router.get("/api/products-map", asyncHandler(async (req, res) => {
  ...
  const payload = isSlim
    ? Object.fromEntries(Object.entries(map).map(([code, p]: [string, any]) => [code, {
        ...
        // 2026-09-09 · barcode 제거 · products.barcode 컬럼 DROP (3fd7d1d9) · 항상 null 반환하던 dead field
        //   · 상품 분류 코드는 product_code 로 통합됨
...
const ALLOWED_INLINE_EDIT = new Set([
  ...
  "brand",
  "manufacturer",
  // 2026-09-09 · barcode 제거 · products.barcode 컬럼 DROP (3fd7d1d9) · PATCH 시 컬럼 없음 에러
  "expiry_date",
  "memo",
