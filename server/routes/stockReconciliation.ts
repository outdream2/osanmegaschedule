...
router.get("/api/stock-reconciliation/:id", async (req, res) => {
  ...
  const [sessionRes, itemsRes] = await Promise.all([
    ...
    supabase
      .from(ITEMS_TABLE)
      .select("id, session_id, product_code, product_name, receiving_qty, invoice_qty, erp_qty, receiving_note, invoice_note, erp_note, receiving_confirmed_by, receiving_confirmed_at, invoice_confirmed_by, invoice_confirmed_at, created_at, updated_at")
      .eq("session_id", id)
