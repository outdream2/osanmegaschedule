...
router.post("/api/upload-vendors", express.raw({ type: "application/octet-stream", limit: "20mb" }), async (req, res) => {
  ...
  try {
    ...
    const normalize = (r: any) => ({
      company_name: String(r.company_name ?? r["공급사"] ?? r["회사명"] ?? r["업체명"] ?? "").trim(),
      contact_name: r.contact_name ?? r["담당자"] ?? r["담당자명"] ?? null,
      phone: r.phone ?? r["전화"] ?? r["전화번호"] ?? r["연락처"] ?? r["휴대폰"] ?? null,
      email: r.email ?? r["이메일"] ?? r["Email"] ?? null,
