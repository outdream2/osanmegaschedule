...
export function xlsxToRows(buf: Buffer): Record<string, any>[] {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // 2026-08-27 · 사용자 지시 · 엑셀 데이터 그대로 DB 반영 · non-numeric current_stock 원본 보존
    //   · current_stock 이 텍스트 ("묶음상품" · "최고할증" 등) 이면 · stock_note 에 원본 저장
    //   · products 테이블에 stock_note 컬럼 있어야 (없으면 upsert 시 자동 필터 · 방어)
    const rawStock = headerIdx.current_stock !== undefined ? row[headerIdx.current_stock] : undefined;
