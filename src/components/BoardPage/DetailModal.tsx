                        ...
                        {editImages.map((img, i) => (
                          <div key={`${img.image_url}-${i}`} className="relative aspect-square rounded-lg overflow-hidden border border-line bg-white">
                            ...
                            <button type="button" onClick={() => removeEditImage(i)}
                              className="absolute top-0.5 right-0.5 w-7 h-7 rounded-full bg-black/70 hover:bg-black text-white text-[15px] font-bold flex items-center justify-center cursor-pointer shadow"
                              title="사진 첨부 취소"
                            >✕</button>
                          </div>
                        ))}
