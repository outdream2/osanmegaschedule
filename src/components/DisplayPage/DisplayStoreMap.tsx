  ...
  const renderAisleSide = (val: number | string, aisleColIdx: number, pairIdx: number, side: "B" | "A") => {
    ...
    return (
      <button
        ...
        className={`${colors.bg} border ${colors.border} rounded flex flex-col items-center justify-start h-full min-h-[76px] px-1 py-1.5 gap-1 cursor-pointer hover:brightness-95 transition overflow-hidden w-full`}
      >
    ...
    <div className="overflow-x-auto">
      <div className="relative p-3 bg-white rounded-2xl flex flex-col border border-line shadow-[0_1px_3px_rgba(15,23,42,0.05),0_8px_24px_-8px_rgba(15,23,42,0.10)] gap-2 min-h-[500px] w-full min-w-[900px] overflow-hidden">
        ...
        */}
        ...
        >
          {/* Row 0: 상단 벽 · col-span-5 · 내부 grid 14셀 · 완벽 균등 크기 · 균일 여백 */}
          <div style={{
            gridColumn: "1 / -1",
            gridRow: 1,
            display: "grid",
            ...
            {STORE_TOP_WALL.map((num, i) => (
              <div key={`tw-${i}`} className="min-w-0 px-1.5 py-1">
          ...
          {/* Row 7: 하단 벽 · col-span-5 · 내부 grid 14셀 · 완벽 균등 크기 · 균일 여백 */}
          <div style={{
            gridColumn: "1 / -1",
            gridRow: 8,
            display: "grid",
            ...
            {STORE_BOTTOM_WALL.map((num, i) => (
              <div key={`bw-${i}`} className="min-w-0 px-1.5 py-1">
