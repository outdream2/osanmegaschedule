...
export const DisplayStoreMap: React.FC<DisplayStoreMapProps> = ({
  ...
  // ─── 좌측 벽면 셀 렌더 ────────────────────────────────────────────────────
  // 2026-09-09 · #14 · 셀 크기 통일 · min-h-[64px] · 격자 균일
  const renderLeftWallCell = (num: number) => {
    ...
    return (
      <button
        ...
        onClick={() => onZoneProductsOpen({ zoneId: String(num), zoneNum: num, zoneLabel: String(num), category: cat })}
        title={`${num} · ${cat} · 클릭 → 상품 조회`}
        className={`${c.bg} border ${c.border} rounded flex flex-col items-center justify-center min-h-[64px] px-1 py-1.5 gap-1 cursor-pointer hover:brightness-95 transition overflow-hidden w-full h-full`}
      >
        <span className={`text-[11px] font-bold text-white ${c.labelBg} rounded px-1.5 py-0.5 leading-none tabular-nums`}>{num}</span>
        <span className={`text-[11px] font-semibold ${c.text} text-center leading-tight break-keep whitespace-normal`}>{cat}</span>
      </button>
    ...
    const renderSide = (val: number | string, side: "B" | "A") => {
      if (isEvent) {
        ...
        return (
          <div
            ...
            className={`${c.bg} border ${c.border} rounded flex flex-col items-center justify-center min-h-[64px] px-1 py-1.5 gap-1 overflow-hidden flex-1`}
          >
      ...
      return (
        <button
          ...
          className={`${colors.bg} border ${colors.border} rounded flex flex-col items-center justify-center min-h-[64px] px-1 py-1.5 gap-1 cursor-pointer hover:brightness-95 transition overflow-hidden flex-1`}
        >
