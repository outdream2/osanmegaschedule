  ...
  return (
    <>
      ...
      {stockImportLog.some(e => !e.start_date) && (
        <div className="text-[12px] text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2 py-1.5 mb-3">
        ...
        <div className="flex flex-wrap items-center gap-1">
          {([
            ...
            <button key={t.k} type="button" onClick={() => setLogFilter(f => ({ ...f, type: t.k }))}
              className={`text-[12px] font-bold rounded-full px-2 py-0.5 border transition cursor-pointer ${logFilter.type === t.k ? `${t.cls} bg-white shadow-sm` : "text-zinc-400 border-line bg-white/60 hover:bg-white"}`}>
              {t.label}
            </button>
          ))}
        </div>
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {(logFilter.type !== "all" || logFilter.from || logFilter.to || logFilter.search) && (
            <button type="button" onClick={() => setLogFilter({ type: "all", from: "", to: "", search: "" })}
              className="text-[12px] text-zinc-400 hover:text-zinc-700 font-bold cursor-pointer">초기화</button>
          )}
        </div>
      </Card>
      {allImportLogs.length === 0
        ...
        : (
          <>
            <div className="text-[12px] text-zinc-400 mb-1.5 px-1"><b className="text-zinc-600">{filtered.length}</b> / {allImportLogs.length} 건</div>
            <Card variant="flat" padding="none" rounded="xl" clip className="max-h-[400px] overflow-y-auto">
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="text-[12px] uppercase tracking-wider text-gray-400 border-b border-line bg-gray-50/70">
