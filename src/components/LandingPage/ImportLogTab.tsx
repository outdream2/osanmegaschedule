  ...
  return (
    <>
      ...
      {stockImportLog.some(e => !e.start_date) && (
        <div className="text-[14px] text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2 py-1.5 mb-3">
      ...
      <Card variant="flat" bg="bg-zinc-50" padding="none" rounded="xl" className="flex flex-col gap-2 mb-3 p-2.5">
        <div className="flex flex-wrap items-center gap-1">
          {([
            ...
            <button key={t.k} type="button" onClick={() => setLogFilter(f => ({ ...f, type: t.k }))}
              className={`text-[14px] font-bold rounded-full px-2 py-0.5 border transition cursor-pointer ${logFilter.type === t.k ? `${t.cls} bg-white shadow-sm` : "text-zinc-400 border-line bg-white/60 hover:bg-white"}`}>
              {t.label}
            </button>
          ))}
        </div>
        <div className="flex flex-wrap items-center gap-1.5">
          <input type="date" value={logFilter.from} onChange={e => setLogFilter(f => ({ ...f, from: e.target.value }))}
            className="text-[15px] tabular-nums border border-line rounded-lg px-2 py-1 bg-white text-zinc-700" title="시작일" />
          ...
          {(logFilter.type !== "all" || logFilter.from || logFilter.to || logFilter.search) && (
            <button type="button" onClick={() => setLogFilter({ type: "all", from: "", to: "", search: "" })}
              className="text-[14px] text-zinc-400 hover:text-zinc-700 font-bold cursor-pointer">초기화</button>
          )}
        </div>
      </Card>
      {allImportLogs.length === 0
        ? <div className="flex flex-col items-center justify-center py-10 text-gray-400 gap-2"><p className="text-base">임포트 이력이 없습니다</p></div>
        : filtered.length === 0
        ? <div className="flex flex-col items-center justify-center py-10 text-gray-400 gap-2"><p className="text-base">필터 조건에 맞는 이력이 없습니다</p></div>
        : (
          <>
            <div className="text-[14px] text-zinc-400 mb-1.5 px-1"><b className="text-zinc-600">{filtered.length}</b> / {allImportLogs.length} 건</div>
            <Card variant="flat" padding="none" rounded="xl" clip className="max-h-[400px] overflow-y-auto">
              <table className="w-full text-[13px]">
                <thead>
                  <tr className="text-[14px] uppercase tracking-wider text-gray-400 border-b border-line bg-gray-50/70">
                  ...
                  {filtered.map((entry, i) => {
                    ...
                    if (entry.kind === "products") return (
                      <tr key={`p-${i}`} className="hover:bg-orange-50/40 transition">
                        <td className="py-1.5 pl-4 pr-3"><span className="text-[14px] font-bold rounded-full px-1.5 py-0.5 border text-orange-700 bg-white border-orange-300">상품</span></td>
                    ...
                    return (
                      <tr key={`pu-${i}`} className="hover:bg-sky-50/40 transition">
                        <td className="py-1.5 pl-4 pr-3"><span className="text-[14px] font-bold rounded-full px-1.5 py-0.5 border text-sky-700 bg-white border-sky-300">매입</span></td>
                        ...
                        <td className="py-1.5 pr-3 text-emerald-700 tabular-nums font-bold" title={entry.endDate}>
                          ...
                          {periodLabel && <span className="ml-1 text-[13px] font-bold px-1 py-0.5 rounded-full border text-purple-700 bg-purple-50 border-purple-200">{periodLabel}</span>}
                        </td>
                        <td className="py-1.5 pr-3 text-right text-gray-500 tabular-nums">{when}</td>
