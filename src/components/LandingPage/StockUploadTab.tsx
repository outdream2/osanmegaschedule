    ...
    <>
      ...
      {stockUploadResult?.ok ? (
        <div className="flex flex-col items-center gap-3 py-4">
          ...
          <p className="text-base font-bold text-emerald-700">임포트 완료</p>
        ...
        <div className="flex flex-col gap-3">
          ...
          <button type="button" onClick={() => stockUploadInputRef.current?.click()}
            className="w-full py-3 border-2 border-dashed border-gray-300 hover:border-indigo-400 text-gray-500 hover:text-indigo-600 text-base font-semibold rounded-xl transition cursor-pointer flex items-center justify-center gap-2">
            <Upload size={16} />
            {stockUploadFile ? stockUploadFile.name : "파일 선택 (.xlsx)"}
          </button>
          {stockUploadResult?.ok === false && (
            <p className="text-sm text-rose-500 font-semibold text-center">{stockUploadResult.msg}</p>
          )}
          <button type="button"
            ...
            className="w-full py-3 bg-brand-deep hover:bg-[#0d3a5c] active:bg-[#08253a] disabled:bg-indigo-200 disabled:cursor-not-allowed text-white font-bold rounded-xl transition cursor-pointer text-base flex items-center justify-center gap-2">
            {stockUploadLoading
