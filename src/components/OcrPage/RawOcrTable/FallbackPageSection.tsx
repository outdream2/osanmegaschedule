        ...
        return (
          <Card key={p.page} variant="flat" padding="none" rounded="2xl" borderColor="border-rose-200" clip className="w-full">
            <div className="px-4 py-2 border-b border-rose-100 bg-rose-50 flex items-center gap-2 flex-wrap">
              <span className="text-[14px] font-bold text-rose-700">
                페이지 {p.page} — 표 감지 실패 (원문)
              </span>
              {supplier && (
                <span className="text-[14px] font-bold text-amber-700">공급: {supplier}</span>
              )}
              <button
                type="button"
                onClick={() => handleMatchPage(p.page)}
                disabled={!!matchingPage[p.page]}
                className="ml-auto inline-flex items-center gap-1 text-[15px] font-bold text-white bg-violet-500 hover:bg-violet-600 disabled:opacity-50 disabled:cursor-not-allowed rounded px-1.5 py-0.5 whitespace-nowrap"
              ...
              {onReparsePage && supplier && (
                <button
                  ...
                  className="inline-flex items-center gap-1 text-[15px] font-bold text-white bg-brand-deep hover:bg-[#0d3a5c] active:bg-[#08253a] rounded px-1.5 py-0.5 whitespace-nowrap"
                  title="이 페이지 재파싱 시도"
                >🔄 재파싱</button>
              )}
            </div>
            <pre className="px-4 py-3 text-[14px] text-gray-600 whitespace-pre-wrap leading-relaxed max-h-48 overflow-y-auto">
