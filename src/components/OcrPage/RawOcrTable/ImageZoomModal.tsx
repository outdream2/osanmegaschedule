  ...
  const zoomControls = (
    <div className="flex items-center gap-2">
      <div className="flex items-center gap-0.5 bg-gray-100 rounded-lg px-1 py-0.5">
        ...
        <span className="text-[14px] font-bold text-gray-500 min-w-[40px] text-center tabular-nums">
          {Math.round(zoom * 100)}%
        </span>
      ...
      <button onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }); }}
        className="text-[15px] font-bold text-gray-400 hover:text-gray-600 px-2 py-1 rounded hover:bg-gray-100 cursor-pointer">
        초기화
      </button>
    </div>
  );
    ...
    >
      ...
      <div className="-m-5 flex flex-col" style={{ height: "calc(90vh - 52px)" }}>
        <div ref={viewportCbRef}
          ...
          {zoom <= 1 && (
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 text-[15px] text-white/70 bg-black/40 px-3 py-1 rounded-full pointer-events-none whitespace-nowrap">
