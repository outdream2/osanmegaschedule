    ...
    >
      <span className={numCellInnerCls}>
        <span className="flex items-center justify-end gap-1">
          ...
          {isCorrectedAmt && <span className="text-[14px] bg-emerald-100 text-emerald-600 px-1 rounded font-bold">보정</span>}
          {dbFilledCells.has(`${ri}-${ci}`) && <span className="text-[14px] bg-indigo-100 text-indigo-700 px-1 rounded font-bold" title="products DB 에서 자동 채움">DB</span>}
...
export const ExpiryCell: React.FC<{
  ...
  return (
    <td key={ci} style={{ minWidth: numCellMinW }}
      ...
      className="px-1 py-2 text-gray-500 text-[15px] group cursor-pointer hover:bg-amber-50/60 text-right"
      title="클릭하여 유통기한 수정">
      <span className={numCellInnerCls}>
        <span className="flex items-center justify-end gap-1">
      ...
      <td key={ci} className="px-3 py-2 max-w-[240px]">
        <div className="flex flex-col gap-0">
          <span className="flex items-center gap-1">
            <span className="text-[14px] bg-emerald-100 text-emerald-700 font-bold px-1 rounded shrink-0">BC</span>
            ...
            <button type="button" title="ERP 자동보정 취소"
              ...
              className="text-[14px] px-1 py-px rounded bg-rose-100 text-rose-600 hover:bg-rose-200 cursor-pointer shrink-0">✕</button>
          </span>
          <span className="text-gray-300 text-[15px] line-through break-words whitespace-normal">{renderTextWithBreaks(String(origCell ?? ""))}</span>
        </div>
      </td>
    );
  }
    ...
    return (
      <td key={ci} className="px-2 py-1.5 max-w-[260px]" onClick={e => e.stopPropagation()}>
        <input
          ...
          className="w-full text-[14px] font-semibold text-gray-800 bg-indigo-50 border border-indigo-300 rounded px-2 py-0.5 outline-none"
  ...
  if (autoMatch) {
    return (
      <td key={ci}
        ...
        <div className="flex flex-col gap-0">
          <span className="flex items-center gap-1">
            <span className="text-[15px] font-bold bg-brand-deep text-white px-1 py-px rounded shrink-0" title="products DB 에서 자동 매칭">DB</span>
          ...
          <button type="button"
            ...
            className="text-gray-300 text-[15px] line-through break-words whitespace-normal hover:text-rose-400 cursor-pointer text-left" title="클릭하여 동의어 삭제">
      ...
      <div className="flex flex-col gap-0.5 items-start">
        ...
        <span className="flex items-center gap-1">
          <button type="button"
            ...
            className={`text-[14px] px-1.5 py-0.5 rounded disabled:opacity-50 cursor-pointer shrink-0 transition font-bold ${
          ...
          {isCancelledAutoMap && (
            <button type="button" title="ERP 자동보정 복원"
              ...
              className="text-[14px] px-1 py-px rounded bg-emerald-100 text-emerald-700 hover:bg-emerald-200 cursor-pointer shrink-0">↩ 복원</button>
          )}
        </span>
      </div>
    </td>
