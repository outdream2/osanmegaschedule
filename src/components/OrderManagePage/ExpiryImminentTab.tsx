...
const dDayCell = (d: number | null): React.ReactNode => {
  ...
  if (d < 0)   return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-red-50 border border-red-200 text-red-700 text-[14px] font-bold">만료 {Math.abs(d)}일 지남</span>;
  if (d === 0) return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-100 border border-amber-300 text-amber-800 text-[14px] font-bold">오늘 만료</span>;
  if (d <= 30) return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-50 border border-amber-200 text-amber-700 text-[14px] font-bold">D-{d}</span>;
  return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-50 border border-emerald-200 text-emerald-700 text-[14px] font-semibold">D-{d}</span>;
};
    ...
    <>
      ...
      <div className="flex flex-col gap-3 p-3 sm:p-4">
        ...
        <div className="flex items-center gap-2 flex-wrap px-1">
          ...
          <span className="text-[17px] tabular-nums font-semibold text-ink-soft">
          ...
          <div className="ml-auto flex items-center gap-1.5">
            ...
            <button
              ...
              onClick={load}
              disabled={loading}
              className="inline-flex items-center gap-1 h-8 px-3 rounded-lg bg-white border border-line text-[15px] font-bold text-ink-soft hover:bg-zinc-50 hover:border-brand-deep hover:text-brand-deep transition cursor-pointer disabled:opacity-40"
              title="새로고침"
            >
        ...
        {loading && rows.length === 0 ? (
          ...
          <Card variant="flat" bg="bg-rose-50" borderColor="border-rose-200" padding="md" className="text-[16px] text-rose-700 font-semibold">
            ⚠ {error}
            ...
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-zinc-100">
                {filtered.map(p => {
                  ...
                  return (
                    <tr key={p.product_code} className={`hover:bg-zinc-50/60 transition text-[16px] ${urgent ? "bg-red-50/30" : ""}`}>
