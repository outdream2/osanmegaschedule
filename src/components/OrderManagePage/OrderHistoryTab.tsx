    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3 flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[15px] font-bold text-slate-800">발주이력</span>
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          <span className="text-[13px] font-semibold text-slate-500">기간</span>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-16 text-slate-400 text-[15px] gap-2">
          ...
          <div className="p-12 text-center text-slate-400 text-[15px]">발주 이력 없음 · 발주 완료 시 여기에 표시</div>
        ) : (
              ...
              return (
                <div key={key} className="hover:bg-slate-50/40 transition">
                  ...
                  >
                    ...
                    <span className="text-[13px] font-semibold text-slate-500 tabular-nums shrink-0 bg-slate-100 rounded-full px-2 py-0.5">
                      {o.items.length}종 · {o.total_qty}개
                    </span>
                    {/* 2026-08-12 · 발주일 · 헤더로 이동 · 상품수 옆 */}
                    {o.order_date && (
                      <span className="inline-flex items-center gap-1 text-[13px] font-medium text-slate-500 tabular-nums shrink-0">
                        <Calendar size={13} className="text-slate-400" />발주 {o.order_date}
                      </span>
                    )}
                    ...
                    {/* 발송 시각 */}
                    <span className="text-[13px] text-slate-400 tabular-nums shrink-0 min-w-[90px] text-right">
                  ...
                  {isOpen && (
                    <div className="px-4 pb-3 space-y-2">
                      ...
                      <div className="flex items-center gap-3 flex-wrap text-[13px] text-slate-500 bg-slate-50/60 border border-slate-100 rounded-lg px-3 py-2">
                        {o.supplier_contact && (
                        ...
                        {!o.supplier_contact && !o.supplier_email && !o.supplier_phone && !o.memo && (
                          <span className="text-slate-300">수신처·메모 정보 없음</span>
                        )}
                      </div>
                      {/* 아이템 테이블 · 폰트 +2 · 굵기 완화 */}
                      <table className="w-full text-[13px] tabular-nums">
                        <thead>
                          <tr className="bg-slate-50 text-slate-500 font-bold tracking-wide text-[12px] border-b border-slate-200">
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {o.items.map((it, i) => (
                            <tr key={it.id} className="hover:bg-indigo-50/30">
                              ...
                              <td className="px-3 py-1.5 font-mono text-slate-500">{it.product_code}</td>
                              <td className="px-3 py-1.5 text-slate-800 font-semibold truncate max-w-[280px]">{it.product_name}</td>
                              <td className="text-right px-3 py-1.5 font-bold text-rose-600">{it.order_qty}</td>
                              <td className="text-right px-3 py-1.5 text-slate-600">{it.unit_price > 0 ? fmtWon(it.unit_price) : "-"}</td>
