...
function recencyStyle(days: number | null): { label: string; cls: string } {
  ...
  return { label: `${days}일전`, cls: "bg-zinc-100 text-zinc-500 border-zinc-300" };
}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 w-full min-w-0">
        <span className={`text-[14px] font-semibold leading-tight flex-1 min-w-0 break-words ${
          active ? "text-sky-800" : "text-zinc-700"
        }`}>
          {product.product_name}
        </span>
        <span
          className={`text-[13px] font-bold rounded px-1.5 py-0.5 border leading-none shrink-0 tabular-nums ${recency.cls}`}
      ...
      <div className="flex items-center gap-2 w-full min-w-0">
        ...
        <span className="text-[14px] font-semibold text-amber-600 tabular-nums shrink-0" title={`매입수량 · ${totalQty.toLocaleString()}개`}>
          {totalQty.toLocaleString()}개
        </span>
        <span className="text-[14px] font-semibold text-zinc-400 tabular-nums shrink-0" title={`매입 건수 · ${purchaseCount}건`}>
          {purchaseCount}건
        </span>
        <span
          className="text-[14px] text-zinc-500 flex-1 min-w-0 whitespace-normal break-words leading-snug"
          title={product.primary_supplier ?? "공급사 정보 없음"}
        >
          {supplierLabel}
        </span>
        {product.product_code && (
          <span className="text-[13px] text-zinc-300 font-mono tabular-nums shrink-0" title={`상품코드 · ${product.product_code}`}>
            {product.product_code}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 w-full min-w-0">
        <span className="text-[14px] font-semibold text-zinc-400 uppercase tracking-wider shrink-0">판매</span>
        <span
        ...
        <span className="text-[14px] font-semibold text-zinc-400 uppercase tracking-wider shrink-0">현재고</span>
        <span
          className={`text-[14px] font-bold tabular-nums shrink-0 ${
