...
export const CategoryPieChart: React.FC<{ rows: PurchaseDetailRow[] }> = ({ rows }) => {
  ...
  if (total === 0) {
    return (
      <div className={`${CARD_BASE} p-4 flex items-center justify-center h-48 text-[15px] text-zinc-400`}>
        데이터 없음
      </div>
    );
  }
  ...
  return (
    <div className={`${CARD_BASE} p-4 flex flex-col gap-3`}>
      <div className="text-[15px] font-bold text-zinc-600 uppercase tracking-wider">
        카테고리별 매입액 비중
      </div>
      ...
      <div className="text-right text-[14px] tabular-nums text-zinc-400 font-semibold">
        합계 {fmtWon(total)}원
      </div>
    </div>
  );
...
const TrendTab: React.FC<{ rows: PurchaseDetailRow[]; loading: boolean }> = ({ rows, loading }) => {
  ...
  if (rows.length === 0) {
    return <div className="flex-1 flex items-center justify-center py-12 text-zinc-400 text-[15px]">해당 기간 매입 데이터 없음</div>;
  }
  ...
  return (
    <div className="flex-1 min-h-0 overflow-auto p-3 flex flex-col gap-3">
      ...
      {pieData.total > 0 && (
        <div className={`${CARD_BASE} p-4 flex flex-col gap-3`}>
          <div className="text-[15px] font-bold text-zinc-600 uppercase tracking-wider">{pieTitle}</div>
  ...
  return (
    <div className={`${CARD_BASE} flex flex-col min-h-0 flex-1`}>
      ...
      <div className="flex flex-wrap items-center border-b border-line bg-zinc-50/50 px-2 pt-1 gap-0">
        ...
        <div className="ml-auto flex items-center gap-1 text-[14px] text-zinc-400 pr-2">
          <ArrowUpDown size={10} />
          <span>헤더 클릭 정렬</span>
        </div>
      </div>
