      ...
      >
        ...
        {!selected ? (
          <div className="bg-white rounded-xl border border-line flex-1 flex flex-col items-center justify-center p-10 text-zinc-400">
            ...
            <div className="text-[15px] mt-1">또는 검색 후 확인 버튼</div>
          </div>
        ) : loading ? (
          ...
          <div className="bg-white rounded-xl border border-line p-10 text-center text-zinc-400 text-sm mx-3 mt-3">
            <div>기간 생성 실패</div>
            <div className="text-[14px] mt-2">#{selected.product_code}</div>
          </div>
        ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button
                      type="button"
                      onClick={() => openScanProductModal({ ...selected, ...(rows[0] ?? {}) })}
                      className="inline-flex items-center gap-1 text-[14px] font-bold text-white bg-brand-deep hover:bg-[#0d3a5c] active:bg-[#08253a] rounded-lg px-2 py-1 cursor-pointer transition shadow-sm shrink-0"
                      title="상세 정보 · 실재고 입력 · 발주 요청"
                    >
                      <Info size={11} /> 정보확인
                    </button>
                  </div>
                  <div className="text-[15px] tabular-nums text-zinc-500">
                    #{selected.product_code} · {rows[0]?.supplier_name ?? selected.supplier ?? "-"}
              ...
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                <div className="flex items-center gap-1.5 ml-auto flex-wrap">
                  <div className="inline-flex items-center gap-1">
                    <span className="text-[15px] font-bold text-teal-700 uppercase tracking-wider">기간</span>
                      ...
                      {[1, 2, 3, 4, 5, 6].map(m => {
                        ...
                        return (
                          <button
                            ...
                            className={`min-w-[24px] px-1.5 py-0.5 text-[14px] font-bold rounded-md transition ${active ? "bg-orange-500 text-white shadow-sm" : "text-zinc-500 hover:text-zinc-800 hover:bg-white"}`}
                          >{m}</button>
                        );
                      })}
                      <span className="text-[15px] font-bold text-zinc-400 self-center px-1">개월</span>
                    </div>
                  </div>
                  <div className="inline-flex items-center gap-1">
                    <span className="text-[15px] font-bold text-teal-700 uppercase tracking-wider">X축</span>
                    <div className="inline-flex bg-zinc-100 border border-line rounded-lg p-0.5">
                      <button
                        onClick={() => onGranularityChange?.("10day")}
                        className={`px-2 py-0.5 text-[14px] font-bold rounded-md transition ${granularity === "10day" ? "bg-orange-500 text-white shadow-sm" : "text-zinc-500 hover:text-zinc-800 hover:bg-white"}`}
                      >10일</button>
                      <button
                        onClick={() => onGranularityChange?.("month")}
                        className={`px-2 py-0.5 text-[14px] font-bold rounded-md transition ${granularity === "month" ? "bg-orange-500 text-white shadow-sm" : "text-zinc-500 hover:text-zinc-800 hover:bg-white"}`}
                      >월</button>
                    </div>
                  </div>
