...
export const InlineField: React.FC<InlineFieldProps> = ({
  ...
  return (
    <div className="min-w-0">
      ...
      {isEditing ? (
        <div className="flex items-center gap-1">
          <input
            ...
            disabled={editSaving}
            autoFocus
            className="flex-1 min-w-0 text-[15px] font-bold border-2 border-indigo-400 rounded px-1.5 py-0.5 focus:outline-none focus:ring-2 focus:ring-brand-tint"
          />
        ...
        <div className="flex items-center gap-1 group">
          <p className={`text-[15px] font-bold ${accentClass} break-words leading-tight flex-1`}>{displayValue}</p>
          {inlineEditEnabled && (
            <button
      ...
      {isEditing && editError && (
        <p className="text-[15px] text-red-500 mt-0.5">{editError}</p>
      )}
    </div>
  );
};
