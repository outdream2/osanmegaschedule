        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-2 mb-2">
            {([
              ...
              <div key={label}>
                <p className="text-[15px] font-semibold text-zinc-500 mb-0.5">{label}</p>
                <p className="text-[15px] font-bold text-zinc-800 break-words leading-tight tabular-nums">{value}</p>
              </div>
            ))}
          </div>
        </>
            ...
            {editingKey === "memo" ? (
              <div className="flex flex-col gap-1">
                <textarea
                  ...
                  autoFocus
                  rows={2}
                  className="w-full text-[15px] font-bold border-2 border-indigo-400 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-brand-tint resize-none"
                />
                <div className="flex items-center gap-1 justify-end">
                  <button onClick={onCommit} disabled={editSaving} className="text-[15px] font-bold text-white bg-brand-deep hover:bg-[#0d3a5c] active:bg-[#08253a] rounded px-2 py-1 flex items-center gap-1 disabled:opacity-40 cursor-pointer">
                    {editSaving ? <Spinner size={11} /> : <Check size={11} />}저장
                  </button>
                  <button onClick={onCancel} disabled={editSaving} className="text-[15px] font-bold text-zinc-600 bg-zinc-200 hover:bg-zinc-300 rounded px-2 py-1 flex items-center gap-1 disabled:opacity-40 cursor-pointer">
                    <X size={11} />취소
                  </button>
                </div>
                {editError && <p className="text-[15px] text-red-500">{editError}</p>}
              </div>
            ) : (
              <div className="flex items-start gap-1 group">
                <p className={`text-[15px] font-bold text-zinc-800 flex-1 whitespace-pre-wrap leading-tight ${!(product as any).memo ? "text-zinc-300 font-bold italic" : ""}`}>
