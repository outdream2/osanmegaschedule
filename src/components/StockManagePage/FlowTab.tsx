      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3 flex flex-wrap items-center gap-x-4 gap-y-2">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {flowMonths > 0 && (() => {
            ...
            return <span className="text-[11px] tabular-nums font-medium text-slate-600 bg-slate-50 border border-slate-200 rounded-md px-2 py-0.5">{s} ~ {e}</span>;
          })()}
        ...
        <div className="flex items-center gap-1.5">
          ...
          <input type="number" min={0} value={salesQtyMin} onChange={e => setSalesQtyMin(e.target.value)} placeholder="최소"
            className="w-14 h-7 px-2 text-[11px] border border-slate-200 rounded-md outline-none focus:ring-1 focus:ring-teal-400 tabular-nums text-right transition" />
                        ...
                        {filteredFlow.map((p, i) => {
                          ...
                          return (
                            <tr key={`flow-${p.product_code}-${i}`} className={`transition ${selectedFlowCodes.has(String(p.product_code)) ? "bg-slate-50" : "hover:bg-slate-50/70"}`}>
                              ...
                              <td className="px-2 py-2.5 align-top">
                                <button type="button" onClick={() => loadFlowSelectedProduct(p)}
                                  ...
                                  {(p as any).min_order != null && (p as any).min_order > 0 && (
                                    <span className="inline-flex items-center ml-1 px-1.5 py-0.5 rounded-sm text-[10px] font-black text-slate-500 bg-slate-100 border border-slate-200 align-middle">
                              ...
                              {/* 재고현황 그룹 */}
                              {!isFlowGroupCollapsed("stock") && <>
                                <td className="text-right px-1.5 py-2.5 font-black text-[14px] bg-slate-50/60 align-top tabular-nums text-slate-700">{fmt(saleV)}</td>
                                {(() => {
                                  ...
                                  return (
                                    <td className={`text-right px-1.5 py-2.5 font-black text-[12px] align-top tabular-nums ${cur <= 0 || mismatch || belowOptimal ? "text-rose-500" : "text-slate-700"}`}
                              ...
                              {isFlowGroupCollapsed("stock") && <td className="bg-slate-50/20" />}
                              {/* 매입현황 그룹 */}
                              {!isFlowGroupCollapsed("purchase") && <>
                                <td className={`text-right px-1.5 py-2.5 font-black text-[12px] align-top tabular-nums ${purchaseCycle != null ? "text-slate-600" : purchaseCount === 1 ? "text-slate-400" : "text-slate-300"}`}
                                ...
                                <td className="text-right px-1.5 py-2.5 text-slate-500 font-black text-[12px] align-top tabular-nums">{lastPDShort}</td>
                                <td className="text-right px-1.5 py-2.5 text-slate-600 font-black text-[12px] align-top tabular-nums">{purchV > 0 ? fmt(purchV) : "-"}</td>
                              </>}
                              {isFlowGroupCollapsed("purchase") && <td className="bg-slate-50/20" />}
                              {/* 판매현황 그룹 */}
                              {!isFlowGroupCollapsed("sales") && <>
                                ...
                                <td className="text-right px-1.5 py-2.5 text-slate-500 font-black text-[12px] align-top tabular-nums">{purP > 0 ? fmtWon(purP) : "-"}</td>
                              ...
                              {isFlowGroupCollapsed("sales") && <td className="bg-slate-50/20" />}
                            </tr>
                          );
                        })}
                      </tbody>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[98vh] sm:max-h-[85vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 bg-amber-50/50">
              ...
              <button onClick={() => setHiddenModalOpen(false)}
                className="text-slate-400 hover:text-slate-700 text-3xl leading-none font-black w-9 h-9 rounded-lg hover:bg-white/70 transition cursor-pointer flex items-center justify-center shrink-0">×</button>
            </div>
            <div className="flex items-center justify-between px-5 py-2.5 border-b border-slate-100 bg-white">
              <span className="text-[11px] font-bold text-slate-500">총 <span className="text-amber-700 font-black">{hiddenList.length}</span>개 숨김</span>
