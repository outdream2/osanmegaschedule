  ...
  <tr className="bg-gradient-to-b from-brand-tint/25 to-brand-tint/10 border-b-2 border-brand-deep/10">
    <td colSpan={99} className="px-0 py-0">
      {loading ? (
        ...
        <div className="py-4 text-center text-[16px] text-zinc-400 font-medium">
          상품 데이터 없음
        </div>
      ) : (
        <div className="px-6 py-3">
          <div className="rounded-lg border border-line bg-white overflow-hidden shadow-[0_1px_2px_rgba(10,46,74,0.04)]">
            <div className="flex items-center gap-2 px-3 py-2 bg-zinc-50/70 border-b border-line">
              ...
              <span className="text-[15px] font-bold text-zinc-700 uppercase tracking-wider">상품 상위 {rows.length}</span>
              <span className="ml-auto text-[14px] text-zinc-400">클릭 시 상세 · 우측 패널</span>
            </div>
              ...
              <tbody className="divide-y divide-zinc-100">
                {rows.slice(0, 20).map((r: any, idx: number) => (
                  <tr key={`${r.product_code}-${idx}`} className="hover:bg-brand-tint/25 transition-colors">
                    <td className="px-3 py-1.5 text-[16px] font-semibold text-ink truncate max-w-[300px]" title={r.product_name}>
                    ...
                    <td className="px-3 py-1.5 text-right tabular-nums text-[16px] font-bold text-brand-deep">
            ...
            {rows.length > 20 && (
              <div className="px-3 py-1.5 bg-zinc-50/40 border-t border-line text-[14px] text-zinc-500 text-center font-medium">
