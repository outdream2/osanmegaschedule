// 2026-08-17 · UI 프레임워크 · Hero banner (brand teal gradient)
...
export function Hero({ eyebrow, title, description, actions, aside, className = "" }: HeroProps) {
  return (
    <div
      ...
      style={{ background: "linear-gradient(120deg, #0A4F44 0%, #0E6B5C 62%, #21987F 100%)" }}
    >
      {/* Decorative blobs */}
      ...
      <div className="relative z-[1] min-w-0 flex-1">
        {eyebrow && (
          <div className="text-[13px] font-bold uppercase tracking-[0.04em] mb-2" style={{ color: "#B9E7DA" }}>
            {eyebrow}
          </div>
        )}
        <h1 className="text-[22px] sm:text-[24px] font-extrabold tracking-[-0.4px] leading-tight m-0 mb-1.5">{title}</h1>
        {description && (
          <p className="text-[14px] sm:text-[15px] m-0" style={{ color: "#CFEBE2" }}>{description}</p>
        )}
        {actions && (
...
/** Hero 내부 CTA · pill button (solid 흰색) */
export function HeroButton({ onClick, icon, children, ghost }: { onClick?: () => void; icon?: ReactNode; children: ReactNode; ghost?: boolean }) {
  ...
  const styleSolid: React.CSSProperties = { background: "#fff", color: "#0A4F44", boxShadow: "0 8px 18px -8px rgba(0,0,0,0.35)" };
