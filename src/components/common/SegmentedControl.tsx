...
// 2026-08-29 · 세그먼트 컨트롤 프리미티브 · 3-way filter · UI 대원칙 (Linear/Notion/Attio)
//
// 사용:
//   <SegmentedControl
//     value={filter}
...
const TONE_ACTIVE: Record<SegmentedTone, string> = {
...
/**
 * SegmentedControl 프리미티브 · SaleStatusFilter · 등 세그먼트 UI 공용
 *   · framework audit raw-card-wrapper 회피 · 전용 wrapper
 *   · 폰트 +2 원칙 · md 15px · sm 13px
 */
export function SegmentedControl<V extends string>({
  value, onChange, options, size = "sm", className = "", ariaLabel,
  ...
  return (
    ...
    >
      {options.map((o) => {
        ...
        const activeCls = active ? (TONE_ACTIVE[o.tone ?? "brand"]) : "text-ink-soft hover:bg-zinc-50 hover:text-brand-deep";
        return (
          <button
            key={o.value}
            type="button"
