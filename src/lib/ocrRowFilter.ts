...
// 2026-07-24 · 사용자 사례 · 라이온코리아 명세서에서 부서명·출고처·요청시간 등이 품명 자리에 나옴
const DELIVERY_INFO_LABELS = [
  ...
  "합계", "소계", "총계", "공급가액", "부가세",
  // 2026-07-24 신규 추가 · 부서·조직·라벨
  "사업부", "영업부", "영업팀", "영업팅", "판매부서", "판매부", "판매팀",
  "출고처", "출고지", "출고일", "출고번호",
  "요청시간", "요청일", "납품일자", "발주일",
  "본사", "지점", "지사", "센터",
  "결제조건", "결제방법", "결제일",
...
export function isNonProductText(text: string): boolean {
  ...
  // 2026-07-24 · 전화번호가 어디든 포함되면 · 상품명 아님 (배송지 정보에 자주 포함)
  //   사용자 사례 · "031-725-9017,9011/동탄상은판매부서"
  if (/0\d{1,2}-\d{3,4}-\d{4}/.test(t)) return true;
  // 2026-07-24 · 슬래시로 여러 항목 나열 (부서/조직 등) · 헤더 오인식 가능성 높음
  if (t.split("/").length >= 2 && t.length > 12) return true;
